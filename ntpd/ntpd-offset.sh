...
REPORT_TEXT="UNKNOWN | NTPd offset is unknown: ${OFFSETS_RAW}"
REPORT_STATUS=${CHECK_STATE_UNKNOWN}

if [ ${NTPDC_STATUS} -ne 0 ]; then
  REPORT_TEXT="ERROR | Error running ntpdc check: ${OFFSETS_RAW}"
  REPORT_STATUS=${CHECK_STATE_ERROR}
else
  for OFFSET in ${OFFSETS}; do
    if [ ${OFFSET:-0} -ge ${NTPD_OFFSET_WARN} ]; then
      REPORT_TEXT="WARN | NTPd offset is high (${OFFSET})"
      REPORT_STATUS=${CHECK_STATE_WARNING}
    elif [ ${OFFSET:-0} -ge ${NTPD_OFFSET_ERROR} ]; then
      REPORT_TEXT="ERROR | NTPd offset is excessive (${OFFSET})"
      REPORT_STATUS=${CHECK_STATE_ERROR}
    else
      REPORT_TEXT="OK | NTPd offset is OK (${OFFSET})"
